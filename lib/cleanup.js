...
const isThisAKeeper = (elementsToKeep, itemsThatWillBeRemoved) => request => {
    if (request.items === undefined)
        if (elementsToKeep.filter(uniqueElement => uniqueElement === request).length == 0) {
            ...
            request.forEachParent(parent => path.push(chalk.red(parent.name)));
            itemsThatWillBeRemoved.add("Path: " + path.reverse().join(chalk.red('/')) + chalk.red('/'+request.name));
            return true;
        } else {
            return false;
        }
...
const cleanup = (sourceFileContent, targetCollectionName) => {
    ...
    console.log(chalk.cyan.bold("The following requests will be deleted from the collection"));
    line();
